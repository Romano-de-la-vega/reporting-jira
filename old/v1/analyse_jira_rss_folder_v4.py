...
def build_summary_sheet(
    ...
    """Page 1 statique et robuste.

    Important: volontairement aucune formule dynamique ici. Les anciennes formules
    LET/FILTER/UNIQUE provoquaient des logs de reparation Excel dans certains
    environnements SharePoint/OneDrive.
    """
    ...
    events_for_summary = list(focus_events or all_events)
    ...
    meta = [
        ...
        ("Périmètre", focus_note),
    ]
    for idx, (label, value) in enumerate(meta, start=4):
        ws[f"A{idx}"] = label
        ws[f"A{idx}"].font = normal_bold
        ws[f"B{idx}"] = value
        ws[f"A{idx}"].fill = PatternFill("solid", fgColor=grey)
        ws[f"A{idx}"].border = border
        ws[f"B{idx}"].border = border
        if isinstance(value, datetime):
            ws[f"B{idx}"].number_format = "yyyy-mm-dd hh:mm"

    ws.merge_cells("D4:N8")
    ws["D4"] = (
        "Cette synthèse est volontairement statique pour garantir l'ouverture Excel. "
        "La vue interactive demandée est en onglet 2 : Slide_2_Story_AC. "
        "Elle permet de choisir le sprint et le statut pour suivre les modifications Story sur Description et Acceptance Criteria."
    )
    ws["D4"].fill = PatternFill("solid", fgColor="EAF2F8")
    ws["D4"].alignment = left
    ws["D4"].border = border

    card_defs = [
        ("Tickets modifiés", count_unique_tickets(events_for_summary), green),
        ("Total modifications", len(events_for_summary), light_blue),
        ("Descriptions modifiées", sum(e.field_name == "Description" for e in events_for_summary), orange),
        ("AC modifiés", sum(e.field_name == "Acceptance Criteria" for e in events_for_summary), orange),
        ("Changements statut", sum(e.field_name == "Statut" for e in events_for_summary), light_blue),
        ("Changements sprint", sum(e.field_name == "Sprint" for e in events_for_summary), light_blue),
        ("Tickets apparus", sum(e.change_type == "Ticket apparu dans l'export" for e in events_for_summary), green),
        ("Tickets disparus", sum(e.change_type == "Ticket disparu de l'export" for e in events_for_summary), orange),
    ]
    card_positions = ["A12", "D12", "G12", "J12", "A16", "D16", "G16", "J16"]
    for (label, value, fill_color), pos in zip(card_defs, card_positions):
        col = ws[pos].column
        row = ws[pos].row
    ...
    field_rows = kpi_by_field(events_for_summary)
    start = 24
    ws[f"A{start}"] = "Modifications par champ"
    ws[f"A{start}"].font = subtitle_font
    headers = ["Champ modifié", "Modifications", "Tickets concernés"]
    for c, h in enumerate(headers, start=1):
        ...
        cell.border = border
    for r_idx, row in enumerate(field_rows[:12], start=start + 2):
    ...
    if field_rows:
        ...
        data = Reference(ws, min_col=2, min_row=start + 1, max_row=start + 1 + min(12, len(field_rows)))
        cats = Reference(ws, min_col=1, min_row=start + 2, max_row=start + 1 + min(12, len(field_rows)))
    ...
    ws["A40"] = "Onglet 2 à utiliser : Slide_2_Story_AC — suivi visuel des modifications Story sur Description et Acceptance Criteria."
    ws["A40"].font = Font(bold=True, color="17365D", size=12)
    ws["A40"].fill = PatternFill("solid", fgColor="FFF2CC")
    ws["A40"].alignment = center
    ws["A40"].border = border
...
def is_story_event(event: ChangeEvent) -> bool:
    return normalize_for_check(event.issue_type_effective) == "story"


def is_story_ticket(ticket: TicketSnapshot) -> bool:
    return normalize_for_check(ticket.issue_type) == "story"


def story_ac_events(events: Sequence[ChangeEvent]) -> List[ChangeEvent]:
    return [
        e for e in events
        if is_story_event(e) and e.field_name in {"Description", "Acceptance Criteria"}
    ]


def collect_story_filters(snapshots: Sequence[SnapshotFile], events: Sequence[ChangeEvent]) -> Tuple[List[str], List[str]]:
    sprints = set()
    statuses = set()
    for e in events:
        if e.sprint_effective:
            sprints.add(e.sprint_effective)
        if e.status_effective:
            statuses.add(e.status_effective)
    for snap in snapshots:
        for t in snap.tickets.values():
            if is_story_ticket(t):
                if t.sprint:
                    sprints.add(t.sprint)
                if t.status:
                    statuses.add(t.status)
    sprint_values = ["Tous"] + sorted(sprints, key=lambda x: x.lower())
    status_values = ["Tous"] + sorted(statuses, key=lambda x: x.lower())
    return sprint_values, status_values


def dim_match_event(e: ChangeEvent, sprint: str, status: str) -> bool:
    return (sprint == "Tous" or e.sprint_effective == sprint) and (status == "Tous" or e.status_effective == status)


def dim_match_ticket(t: TicketSnapshot, sprint: str, status: str) -> bool:
    return is_story_ticket(t) and (sprint == "Tous" or t.sprint == sprint) and (status == "Tous" or t.status == status)


def build_story_summary_maps(snapshots: Sequence[SnapshotFile]) -> Tuple[Dict[str, str], Dict[str, TicketSnapshot]]:
    summary_by_id: Dict[str, str] = {}
    latest_by_id: Dict[str, TicketSnapshot] = {}
    for snap in snapshots:
        for ticket_id, ticket in snap.tickets.items():
            summary_by_id[ticket_id] = ticket.summary
            latest_by_id[ticket_id] = ticket
    return summary_by_id, latest_by_id


def create_hidden_sheet(wb: Any, name: str, rows: List[List[Any]]) -> Any:
...
def build_story_ac_support_sheets(wb: Any, snapshots: Sequence[SnapshotFile], events: Sequence[ChangeEvent]) -> Dict[str, str]:
    """Construit les feuilles support de la slide 2.

    Les calculs lourds sont faits en Python. Excel n'utilise ensuite que des formules
    INDEX/MATCH/SUMIFS simples, compatibles avec les versions Excel classiques.
    """
    Workbook, BarChart, LineChart, Reference, Alignment, Border, Font, PatternFill, Side, get_column_letter, DataValidation = import_openpyxl()
    story_events = story_ac_events(events)
    sprint_values, status_values = collect_story_filters(snapshots, story_events)
    summary_by_id, latest_by_id = build_story_summary_maps(snapshots)
    latest_snapshot = snapshots[-1] if snapshots else None
    latest_tickets = list(latest_snapshot.tickets.values()) if latest_snapshot else []
    window_dates = sorted({e.window_end for e in events})
    if not window_dates and snapshots:
        window_dates = [snapshots[-1].export_date]

    # Listes de validation.
    list_rows = [["Sprint", "Statut"]]
    max_len = max(len(sprint_values), len(status_values))
    for i in range(max_len):
        list_rows.append([
            sprint_values[i] if i < len(sprint_values) else None,
            status_values[i] if i < len(status_values) else None,
        ])
    create_hidden_sheet(wb, "_Story_AC_Lists", list_rows)

    # KPI par Sprint / Statut.
    kpi_rows = [[
        "Key", "Sprint", "Statut", "Stories dernier export", "Tickets Story impactés",
        "Total modifications", "Descriptions modifiées", "AC modifiés", "Tickets desc modifiée",
        "Tickets AC modifiés", "Tickets desc+AC", "Taux impact", "Première détection", "Dernière détection"
    ]]
    field_rows = [["Key", "Sprint", "Statut", "Champ", "Modifications", "Tickets concernés"]]
    evo_rows = [["Key", "Sprint", "Statut", "Date clé", "Descriptions modifiées", "AC modifiés", "Tickets impactés"]]
    top_rows = [[
        "Key", "Sprint", "Statut", "Rang", "Ticket ID", "Jira key", "Résumé", "Statut ticket",
        "Description", "AC", "Total", "Dernière détection", "Lien"
    ]]
    recent_rows = [[
        "Key", "Sprint", "Statut", "Rang", "Détection", "Jira key", "Champ", "Statut ticket",
        "Sprint ticket", "Extrait avant", "Extrait après", "Lien", "Résumé"
    ]]

    for sprint in sprint_values:
        for status in status_values:
            dim_key = f"{sprint}|{status}"
            dim_events = [e for e in story_events if dim_match_event(e, sprint, status)]
            latest_dim_tickets = [t for t in latest_tickets if dim_match_ticket(t, sprint, status)]
            impacted = {e.ticket_id for e in dim_events}
            desc_events = [e for e in dim_events if e.field_name == "Description"]
            ac_events = [e for e in dim_events if e.field_name == "Acceptance Criteria"]
            desc_tickets = {e.ticket_id for e in desc_events}
            ac_tickets = {e.ticket_id for e in ac_events}
            first_det = min((e.window_end for e in dim_events), default=None)
            last_det = max((e.window_end for e in dim_events), default=None)
            denom = len({t.ticket_id for t in latest_dim_tickets})
            impact_rate = round(len(impacted) / denom, 4) if denom else 0
            kpi_rows.append([
                dim_key, sprint, status, denom, len(impacted), len(dim_events), len(desc_events), len(ac_events),
                len(desc_tickets), len(ac_tickets), len(desc_tickets & ac_tickets), impact_rate,
                excel_dt(first_det), excel_dt(last_det),
            ])
            for field_name in ["Description", "Acceptance Criteria"]:
                field_dim_events = [e for e in dim_events if e.field_name == field_name]
                field_rows.append([
                    f"{dim_key}|{field_name}", sprint, status, field_name,
                    len(field_dim_events), count_unique_tickets(field_dim_events),
                ])
            for dt in window_dates:
                dt_key = format_dt(dt)
                win_events = [e for e in dim_events if e.window_end == dt]
                evo_rows.append([
                    f"{dim_key}|{dt_key}", sprint, status, dt_key,
                    sum(e.field_name == "Description" for e in win_events),
                    sum(e.field_name == "Acceptance Criteria" for e in win_events),
                    count_unique_tickets(win_events),
                ])
            ...
            for ticket_id, items in grouped.items():
                latest_event = max(items, key=lambda e: e.window_end)
                desc_count = sum(e.field_name == "Description" for e in items)
                ac_count = sum(e.field_name == "Acceptance Criteria" for e in items)
                top_items.append((ticket_id, items, latest_event, desc_count, ac_count))
            top_items.sort(key=lambda x: (-(x[3] + x[4]), -x[3], -x[4], x[2].window_end, x[2].key_after or x[2].key_before))
            for rank, (ticket_id, items, latest_event, desc_count, ac_count) in enumerate(top_items[:15], start=1):
                latest_ticket = latest_by_id.get(ticket_id)
                jira_key = latest_event.key_after or latest_event.key_before
                top_rows.append([
                    f"{dim_key}|{rank}", sprint, status, rank, ticket_id, jira_key,
                    summary_by_id.get(ticket_id, ""),
                    latest_ticket.status if latest_ticket else latest_event.status_effective,
                    desc_count, ac_count, desc_count + ac_count, excel_dt(latest_event.window_end), latest_event.link,
                ])

            recent_items = sorted(dim_events, key=lambda e: (e.window_end, e.key_after or e.key_before, e.field_name), reverse=True)
            for rank, e in enumerate(recent_items[:15], start=1):
                recent_rows.append([
                    f"{dim_key}|{rank}", sprint, status, rank, excel_dt(e.window_end),
                    e.key_after or e.key_before, e.field_name, e.status_effective, e.sprint_effective,
                    e.before_excerpt, e.after_excerpt, e.link, summary_by_id.get(e.ticket_id, ""),
                ])

    create_hidden_sheet(wb, "_Story_AC_KPI", kpi_rows)
    create_hidden_sheet(wb, "_Story_AC_Field", field_rows)
    create_hidden_sheet(wb, "_Story_AC_Evo", evo_rows)
    create_hidden_sheet(wb, "_Story_AC_Top", top_rows)
    create_hidden_sheet(wb, "_Story_AC_Recent", recent_rows)

    list_sheet = excel_quote_sheet("_Story_AC_Lists")
    return {
        "sprint_validation": f"={list_sheet}!$A$2:$A${len(sprint_values)+1}",
        "status_validation": f"={list_sheet}!$B$2:$B${len(status_values)+1}",
        "window_dates": [format_dt(dt) for dt in window_dates],
    }


def idx_match_formula(sheet_name: str, key_expr: str, return_col: str, default: str = "0") -> str:
    sh = excel_quote_sheet(sheet_name)
    return f'=IFERROR(INDEX({sh}!${return_col}:${return_col},MATCH({key_expr},{sh}!$A:$A,0)),{default})'


def text_idx_match_formula(sheet_name: str, key_expr: str, return_col: str) -> str:
    return idx_match_formula(sheet_name, key_expr, return_col, '""')


def build_story_ac_slide(wb: Any, snapshots: Sequence[SnapshotFile], all_events: Sequence[ChangeEvent]) -> None:
    Workbook, BarChart, LineChart, Reference, Alignment, Border, Font, PatternFill, Side, get_column_letter, _ = import_openpyxl()
    support = build_story_ac_support_sheets(wb, snapshots, all_events)
    ws = wb.create_sheet("Slide_2_Story_AC")
    ws.sheet_view.showGridLines = False
    ...
    white_font = Font(color="FFFFFF", bold=True)
    title_font = Font(color="FFFFFF", bold=True, size=18)
    subtitle_font = Font(color="17365D", bold=True, size=12)
    center = Alignment(horizontal="center", vertical="center", wrap_text=True)
    left = Alignment(horizontal="left", vertical="top", wrap_text=True)
    thin = Side(style="thin", color="BFBFBF")
    border = Border(left=thin, right=thin, top=thin, bottom=thin)
    ...
    ws.merge_cells("A1:N2")
    ws["A1"] = "Slide 2 - Évolution des Stories : Description & Acceptance Criteria"
    ...
    ws["A4"] = "Objectif\nSuivre, par sprint, les changements intervenus sur les tickets de type Story pour les champs Description et Acceptance Criteria."
    ws["A4"].alignment = left
    ws["A4"].fill = PatternFill("solid", fgColor="EAF2F8")
    ws["A4"].border = border

    ws["D4"] = "Sprint à analyser"
    ws["D4"].fill = PatternFill("solid", fgColor=blue)
    ws["D4"].font = white_font
    ws["D4"].alignment = center
    ws["E4"] = "Tous"
    ws["E4"].fill = PatternFill("solid", fgColor=yellow)
    ws["E4"].font = Font(bold=True, color="17365D")
    ws["E4"].alignment = center
    add_dropdown(ws, "E4", support["sprint_validation"])

    ws["H4"] = "Statut"
    ws["H4"].fill = PatternFill("solid", fgColor=blue)
    ws["H4"].font = white_font
    ws["H4"].alignment = center
    ws["I4"] = "Tous"
    ws["I4"].fill = PatternFill("solid", fgColor=yellow)
    ws["I4"].font = Font(bold=True, color="17365D")
    ws["I4"].alignment = center
    add_dropdown(ws, "I4", support["status_validation"])

    ws.merge_cells("D6:N7")
    ws["D6"] = (
        "Sélectionne un sprint en E4 et, si besoin, un statut en I4. "
        "La page utilise uniquement des formules Excel compatibles : INDEX/MATCH et SUMIFS. "
        "Type de ticket fixé : Story. Champs suivis : Description et Acceptance Criteria."
    )
    ws["D6"].alignment = left
    ws["D6"].fill = PatternFill("solid", fgColor="F8FBFD")
    ws["D6"].border = border

    for cell in ["D4", "E4", "H4", "I4"]:
        ws[cell].border = border

    # KPI cards.
    key_expr = '$E$4&"|"&$I$4'
    card_defs = [
        ("Stories dans le dernier export", idx_match_formula("_Story_AC_KPI", key_expr, "D"), light_blue, "0"),
        ("Stories impactées", idx_match_formula("_Story_AC_KPI", key_expr, "E"), green, "0"),
        ("Total modifs Desc/AC", idx_match_formula("_Story_AC_KPI", key_expr, "F"), light_blue, "0"),
        ("Descriptions modifiées", idx_match_formula("_Story_AC_KPI", key_expr, "G"), orange, "0"),
        ("AC modifiés", idx_match_formula("_Story_AC_KPI", key_expr, "H"), orange, "0"),
        ("Stories avec Desc + AC", idx_match_formula("_Story_AC_KPI", key_expr, "K"), green, "0"),
        ("Taux de stories impactées", idx_match_formula("_Story_AC_KPI", key_expr, "L"), yellow, "0%"),
        ("Dernière détection", text_idx_match_formula("_Story_AC_KPI", key_expr, "N"), light_blue, "yyyy-mm-dd hh:mm"),
    ]
    positions = ["A10", "D10", "G10", "J10", "A14", "D14", "G14", "J14"]
    for (label, formula, fill_color, number_format), pos in zip(card_defs, positions):
        col = ws[pos].column
        row = ws[pos].row
        ws.merge_cells(start_row=row, start_column=col, end_row=row, end_column=col + 1)
        ws.merge_cells(start_row=row + 1, start_column=col, end_row=row + 2, end_column=col + 1)
        ...
        bottom.fill = PatternFill("solid", fgColor=fill_color)
        bottom.font = Font(bold=True, size=17, color="17365D")
        bottom.alignment = center
        if number_format:
            bottom.number_format = number_format
        for rr in range(row, row + 3):
            for cc in range(col, col + 2):
                ws.cell(rr, cc).border = border

    # Source graphique champ.
    chart_row = 20
    ws[f"A{chart_row}"] = "Répartition des changements Story"
    ws[f"A{chart_row}"].font = subtitle_font
    headers = ["Champ", "Modifications", "Tickets concernés"]
    for c, h in enumerate(headers, start=1):
    ...
    for i, field_name in enumerate(fields, start=chart_row + 2):
        ws.cell(i, 1).value = field_name
        fkey = f'$E$4&"|"&$I$4&"|"&A{i}'
        ws.cell(i, 2).value = idx_match_formula("_Story_AC_Field", fkey, "E")
        ws.cell(i, 3).value = idx_match_formula("_Story_AC_Field", fkey, "F")
        for c in range(1, 4):
            ws.cell(i, c).border = border

    chart = BarChart()
    chart.title = "Description vs Acceptance Criteria"
    chart.y_axis.title = "Modifications"
    data = Reference(ws, min_col=2, min_row=chart_row + 1, max_row=chart_row + 3)
    cats = Reference(ws, min_col=1, min_row=chart_row + 2, max_row=chart_row + 3)
    chart.add_data(data, titles_from_data=True)
    chart.set_categories(cats)
    chart.height = 8
    chart.width = 13
    ...
    evo_row = 34
    ws[f"A{evo_row}"] = "Évolution par fenêtre d'export"
    ws[f"A{evo_row}"].font = subtitle_font
    evo_headers = ["Date de détection", "Descriptions", "AC", "Tickets impactés"]
    for c, h in enumerate(evo_headers, start=1):
        cell = ws.cell(evo_row + 1, c)
        cell.value = h
        cell.fill = PatternFill("solid", fgColor=blue)
        cell.font = white_font
        cell.alignment = center
        cell.border = border
    date_keys = support.get("window_dates", [])[:30]
    if not date_keys:
        date_keys = [""]
    for idx, date_key in enumerate(date_keys, start=evo_row + 2):
        ws.cell(idx, 1).value = date_key
        base_key = f'$E$4&"|"&$I$4&"|"&A{idx}'
        ws.cell(idx, 2).value = idx_match_formula("_Story_AC_Evo", base_key, "E")
        ws.cell(idx, 3).value = idx_match_formula("_Story_AC_Evo", base_key, "F")
        ws.cell(idx, 4).value = idx_match_formula("_Story_AC_Evo", base_key, "G")
        for c in range(1, 5):
            ws.cell(idx, c).border = border

    if date_keys:
        line = LineChart()
        line.title = "Tendance des modifications Story"
        line.y_axis.title = "Nombre"
        data = Reference(ws, min_col=2, max_col=4, min_row=evo_row + 1, max_row=evo_row + 1 + len(date_keys))
        cats = Reference(ws, min_col=1, min_row=evo_row + 2, max_row=evo_row + 1 + len(date_keys))
        line.add_data(data, titles_from_data=True)
        line.set_categories(cats)
        line.height = 9
        line.width = 17
        ws.add_chart(line, "F34")

    # Top tickets.
    top_row = 68
    ws[f"A{top_row}"] = "Top tickets Story modifiés sur Description / AC"
    ws[f"A{top_row}"].font = subtitle_font
    top_headers = ["Rang", "Key", "Statut", "Desc", "AC", "Total", "Dernière détection", "Résumé", "Lien"]
    for c, h in enumerate(top_headers, start=1):
        cell = ws.cell(top_row + 1, c)
        cell.value = h
        cell.fill = PatternFill("solid", fgColor=blue)
        cell.font = white_font
        cell.alignment = center
    ...
    for r in range(1, 11):
        row = top_row + 1 + r
        ws.cell(row, 1).value = r
        tkey = f'$E$4&"|"&$I$4&"|"&A{row}'
        formulas = [
            text_idx_match_formula("_Story_AC_Top", tkey, "F"),
            text_idx_match_formula("_Story_AC_Top", tkey, "H"),
            idx_match_formula("_Story_AC_Top", tkey, "I"),
            idx_match_formula("_Story_AC_Top", tkey, "J"),
            idx_match_formula("_Story_AC_Top", tkey, "K"),
            text_idx_match_formula("_Story_AC_Top", tkey, "L"),
            text_idx_match_formula("_Story_AC_Top", tkey, "G"),
            text_idx_match_formula("_Story_AC_Top", tkey, "M"),
        ]
        for c_idx, formula in enumerate(formulas, start=2):
            ws.cell(row, c_idx).value = formula
        for c in range(1, len(top_headers) + 1):
            ws.cell(row, c).border = border
            ws.cell(row, c).alignment = left if c in {8, 9} else center

    # Derniers changements, avec extraits avant/après.
    recent_row = 83
    ws[f"A{recent_row}"] = "Derniers changements détectés - extraits avant / après"
    ws[f"A{recent_row}"].font = subtitle_font
    recent_headers = ["Rang", "Détection", "Key", "Champ", "Statut", "Extrait avant", "Extrait après", "Lien"]
    for c, h in enumerate(recent_headers, start=1):
        cell = ws.cell(recent_row + 1, c)
        cell.value = h
        cell.fill = PatternFill("solid", fgColor=blue)
        cell.font = white_font
        cell.alignment = center
    ...
    for r in range(1, 11):
        row = recent_row + 1 + r
        ws.cell(row, 1).value = r
        rkey = f'$E$4&"|"&$I$4&"|"&A{row}'
        formulas = [
            text_idx_match_formula("_Story_AC_Recent", rkey, "E"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "F"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "G"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "H"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "J"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "K"),
            text_idx_match_formula("_Story_AC_Recent", rkey, "L"),
        ]
        for c_idx, formula in enumerate(formulas, start=2):
            ws.cell(row, c_idx).value = formula
        for c in range(1, len(recent_headers) + 1):
            ws.cell(row, c).border = border
            ws.cell(row, c).alignment = left if c in {6, 7, 8} else center
...
def build_excel_report(
    ...
    # Synthèse en premier onglet, sans formules dynamiques fragiles.
    ...
    # Slide 2 : vue specialisee Stories / Description / AC.
    build_story_ac_slide(wb, snapshots, all_period_events)
    story_ws = wb["Slide_2_Story_AC"]
    wb._sheets.remove(story_ws)
    wb._sheets.insert(1, story_ws)
    ...
    sheets = {
        ...
        "Story_Desc_AC_events": [e.as_row() for e in story_ac_events(all_period_events)],
